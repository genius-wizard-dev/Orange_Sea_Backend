...
//     try {
//       this.logger.log({
//         message: 'Bắt đầu xử lý ngắt kết nối',
//         socketId: client.id,
//       });
...
//       this.logger.log({
//         message: 'Thông tin socket khi ngắt kết nối',
//         socketId: client.id,
//         profileId,
//         activeGroupId,
//       });
...
//       if (!profileId) {
//         this.logger.warn({
//           message: 'Không tìm thấy profileId cho socket khi ngắt kết nối',
//           socketId: client.id,
//         });
//         return;
//       }
...
//       // Kiểm tra xem socket có tồn tại trong danh sách kết nối không
//       const isMember = await this.redisService.sismember(connectionsKey, client.id);
//       if (!isMember) {
//         this.logger.warn({
//           message: 'Socket không tồn tại trong danh sách kết nối',
//           socketId: client.id,
//           profileId,
//         });
//       }

//       // Tạo pipeline để thực hiện nhiều thao tác Redis cùng lúc
//       const pipeline = this.redisService.multi();

//       // Xóa socket khỏi danh sách kết nối của profile
//       pipeline.srem(connectionsKey, client.id);
//       pipeline.del(socketToProfileKey);

//       // Xử lý dọn dẹp nhóm đang xem nếu socket đang xem một nhóm
//       if (activeGroupId) {
//         const viewersKey = `${this.ACTIVE_GROUP_VIEWERS}${activeGroupId}`;
//         pipeline.srem(viewersKey, profileId);
//         pipeline.del(socketToGroupKey);

//         // Thông báo cho nhóm về việc người dùng không còn hoạt động
...
//       // Thực thi tất cả các lệnh Redis
//       const pipelineResults = await pipeline.exec();

//       this.logger.log({
//         message: 'Kết quả thực thi pipeline Redis',
//         pipelineResults,
//         socketId: client.id,
//       });

//       // Kiểm tra các kết nối còn lại sau khi pipeline đã thực thi
//       const remainingConnections = await this.redisService.scard(connectionsKey);

//       this.logger.log({
//         message: 'Số kết nối còn lại',
//         remainingConnections,
//         profileId,
//       });

//       if (remainingConnections === 0) {
//         // Nếu không còn kết nối nào, dọn dẹp tập hợp kết nối
//         await this.redisService.del(connectionsKey);
//         await this.broadcastUserStatus(profileId, false);

//         this.logger.log({
//           message: 'Đã xóa tất cả kết nối của người dùng',
//           profileId,
//         });
//       } else {
//         // Người dùng vẫn còn các kết nối hoạt động khác
//         if (activeGroupId) {
//           this.server.to(activeGroupId).emit('userStatusUpdate', {
//             profileId,
//             isActive: false,
...
//       // Kiểm tra xem còn người xem nào trong nhóm không
//       if (activeGroupId) {
//         const viewersKey = `${this.ACTIVE_GROUP_VIEWERS}${activeGroupId}`;
//         const remainingViewers = await this.redisService.smembers(viewersKey);

//         this.logger.log({
//           message: 'Số người xem còn lại trong nhóm',
//           groupId: activeGroupId,
...
//           this.logger.log({
//             message: 'Đã xóa danh sách người xem của nhóm',
//             groupId: activeGroupId,
//           });
//         }
//       }
...
//       this.logger.log({
//         message: 'Hoàn tất xử lý ngắt kết nối',
//         socketId: client.id,
//         profileId,
//       });
//     } catch (error) {
//       this.logger.error({
//         message: 'Lỗi khi xử lý ngắt kết nối',
//         error: error.message,
//         stack: error.stack,
//         socketId: client.id,
//       });

//       // Thử xóa lại các key Redis trong trường hợp lỗi
//       try {
//         const socketToProfileKey = `${this.SOCKET_TO_PROFILE}${client.id}`;
//         const profileId = await this.redisService.get<string>(socketToProfileKey);

//         if (profileId) {
//           const connectionsKey = `${this.PROFILE_CONNECTIONS}${profileId}`;
//           await this.redisService.srem(connectionsKey, client.id);
//           await this.redisService.del(socketToProfileKey);

//           const socketToGroupKey = `${this.SOCKET_TO_ACTIVE_GROUP}${client.id}`;
//           await this.redisService.del(socketToGroupKey);

//           this.logger.log({
//             message: 'Đã xóa dữ liệu Redis sau khi xử lý lỗi',
//             socketId: client.id,
//             profileId,
//           });
//         }
//       } catch (cleanupError) {
//         this.logger.error({
//           message: 'Lỗi khi cố gắng dọn dẹp Redis sau lỗi',
