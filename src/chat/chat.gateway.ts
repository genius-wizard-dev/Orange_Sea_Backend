...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleRecallMessage(
    @MessageBody(new ValidationPipe()) data: RecallMessageDto,
  ) {
    try {
      const { messageId, groupId, senderId } = data;
      if (!messageId || !groupId || !senderId)
  ...
  async handleEditMessage(
    @MessageBody(new ValidationPipe()) data: EditMessageSocketDto,
    @ConnectedSocket() client: Socket,
  ) {
    try {
  ...
  async handleGetActiveUsers(
    @MessageBody(new ValidationPipe()) data: GetActiveUsersDto,
    @ConnectedSocket() client: Socket,
  ) {
    try {
      const { groupId } = data;
  ...
  async handleGetUnreadCounts(
    @MessageBody(new ValidationPipe()) data: GetUnreadCountsDto,
    @ConnectedSocket() client: Socket,
  ) {
    try {
      const { profileId } = data;
